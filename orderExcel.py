...
def order_excel(check_in, check_out):
    ...
    query = """
        SELECT
            ...
            dbo.deduction / 100.0 AS '分销商退款金额',
            dpt.pay_time AS '退款时间',
            dso.brokerage / 100.0 AS '佣金',
            dbo.deduction / 100.0 AS '供应商扣款',
            dbo.seller_price / 100.0 AS '分销商扣款',
            dbo.price / 100.0 AS '售卖价',
            dbo.tp_status / 100.0 AS '结算价',
            dbo.pft_status / 100.0 AS '利润',
            'CNY' AS '币种',
