...
def fliggy_bulu(order_data, is_busy, device_id, bg_order_id, biz_order_id, price, device_name, supplier_id):
    try:
        if supplier_id == '10002':
            is_busy += 1
            set_not_effective_device(device_id, 1, is_busy)
        create_order_res = hybridization_create_order(order_data, bg_order_id, biz_order_id, price, device_id,
                                                      supplier_id)
...
def timeout_main(order_data, start_time, device_id, tar_json, is_busy, bg_order_id, device_name):
    if time.time() - start_time > 15:
        try:
            ...
            if order_res:
                ...
                fliggy_bulu(order_data, is_busy, device_id, bg_order_id, biz_order_id, price, device_name, "10002")
                return True
            else:
                return False
        except Exception as f:
...
def axin_pay(order_res, bg_order_id, device_name):
    if order_res['sOrderId']:
        if pay_axin(order_res['sOrderId']):
            fliggy_bulu(order_res, None, device_id, bg_order_id, None, None,
                        device_name, "10004")
    else:
        fail_order_unlock(0, 1, bg_order_id, device_id, device_name)
