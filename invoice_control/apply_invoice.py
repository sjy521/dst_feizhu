            ...
            for res_data in res_json['data']['order_list']:
                if res_data['order_time'] is not None:
                    if res_data['order_time'] < "{} 00:00:00".format(
                            (datetime.datetime.now() - datetime.timedelta(days=8)).date().strftime('%Y-%m-%d')):
                        exceed_time_num -= 1
...
if __name__ == '__main__':
    ...
    for res in all_device:
        if res.get("deviceId") == device_id:
            ...
            file_name = "{}{}发票记录".format((datetime.datetime.now() - datetime.timedelta(days=8)).date().strftime('%Y-%m-%d'), device_name)
